...
*/
		...
		Loop, Parse, sIni, `n, `r
		{
			...
			if (SubStr(sTrimmedLine, 1, 1) == ";" || sTrimmedLine == "") ; "" would be a newline
			{
				...
				;~ LoopField :=v== "" ? Chr(14) : v

				if (sCurSec == "")
					this.EasyIni_ReservedFor_TopComments.Insert(A_Index, LoopField) ; not using sTrimmedLine so as to keep comment formatting
				else
				{
					if (sPrevKeyForThisSec == "") ; This happens when there is a comment in the section before the first key, if any
			...
			{
				if (sCurSec != "" && !this.HasKey(sCurSec))
	...
	{
		for _key in this[sec]
			keys .= A_Index == 1 ? _key : sDelim _key
	...
	Copy(SourceIni, bCopyFileName = true)
	{
		...
		if (IsObject(this))
		{
			...
			this := "" ; avoid any copy constructor issues.
	...
	*/
	Merge(vOtherIni, bRemoveNonMatching = false, bOverwriteMatching = false, vExceptionsIni = "")
	{
		...
		for sec, aKeysToVals in vOtherIni
		{
			...
			; key=val
			for _key, val in aKeysToVals
			{
				bMakeException := vExceptionsIni[sec].HasKey(_key)

				if (this[sec].HasKey(_key))
				{
				...
				else
				{
					if (bRemoveNonMatching && !bMakeException)
						this.DeleteKey(sec, _key)
					else if (!bRemoveNonMatching)
						this.AddKey(sec, _key, val)
				}
			}
		}
		return
	...
	*/
	...
	Save(sSaveAs="", bWarnIfExist=false)
	{
		...
		for section, aKeysToVals in this
		{
			...
			{
				bEmptySection := false
				sLastAddedLine := "`n" _key "=" val
				; FileAppend, %sLastAddedLine%, %sFile%, UTF-8
				initext .= sLastAddedLine

				; Add the comment(s) for this key
				sComments := this[section].EasyIni_ReservedFor_Comments[_key]
				Loop, Parse, sComments, `n
				{
					sLastAddedLine := "`n" (A_LoopField == Chr(14) ? "" : A_LoopField)
					; FileAppend, %sLastAddedLine%, %sFile%, UTF-8
