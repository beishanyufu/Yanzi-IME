...
pgfile:=RegExReplace(A_ScriptDir, "\\[^\\]+\\[^\\]+$")
If (A_Args[1]="EnableUIAccess"){
	If !InStr(A_ScriptFullPath, Program:=StrReplace(A_ProgramFiles, " (x86)")){
		FileRemoveDir % Program "\Yzime"
		Run *RunAs cmd.exe /c mklink /D "%Program%\Yzime" "%pgfile%", , Hide, PID
	}
	...
	} Else If !FileExist(pgfile "\Yzime_UIA.exe"){
		FileCopy, %A_AhkPath%, % pgfile "\Yzime_UIA.exe"
		EnableUIAccess(pgfile "\Yzime_UIA.exe")
	}
	Loop, Files, %pgfile%\Plugins\*.exe
		If !InStr(A_LoopFileFullPath, "_UIA.exe"){
			FileCopy, %A_LoopFileFullPath%, % SubStr(A_LoopFileFullPath, 1, -4) "_UIA.exe"
			EnableUIAccess(SubStr(A_LoopFileFullPath, 1, -4) "_UIA.exe")
		}
	WinWaitClose, ahk_pid%PID%
} Else {
	For subfile In {"":"","\Plugins":""}
		Loop, Files, %pgfile%%subfile%\*.exe
			If (A_LoopFileFullPath~="_UIA\.exe$")
				FileDelete %A_LoopFileFullPath%
	FileRemoveDir % StrReplace(A_ProgramFiles, " (x86)") "\Yzime"
